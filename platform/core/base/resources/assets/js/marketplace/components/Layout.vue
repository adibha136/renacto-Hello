<template>
    <div class="card-body">
        <div v-if="connection">
            <div class="row">
                <div class="col-md-6 col-sm-12">
                    <ul class="nav nav-pills">
                        <li class="nav-item">
                            <a class="nav-link" :class="{ active: tab === 'all' }" @click="changeTab('all')">{{
                                    __('base.all')
                                }} <span v-if="pagination.total">({{ pagination.total }})</span></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" :class="{ active: tab === 'featured' }" @click="changeTab('featured')">{{
                                    __('base.featured')
                                }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" :class="{ active: tab === 'popular' }" @click="changeTab('popular')">{{
                                    __('base.popular')
                                }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" :class="{ active: tab === 'top_rated' }"
                               @click="changeTab('top_rated')">{{
                                    __('base.top_rated')
                                }}</a>
                        </li>
                    </ul>
                ...
                <div class="col-md-6 col-sm-12">
                    <search :keyword="keyword" @search="search"></search>
                </div>
            </div>
            ...
            <pagination v-if="!loading" :pagination="pagination" @change-page="changePage"></pagination>

            <modal id="plugin-detail" ref="pluginDetail" :iframe-url="productIframeUrl"></modal>

        </div>

        <div v-else class="card-body text-center">
            <h1 class="mt-20 mb-20">{{ __('base.connection_aborted') }}</h1>
            <p class="fw-lighter">{{ __('base.connection_aborted_description') }}</p>

            <span class="mt-5 img-fluid rounded mx-auto d-block">
                <svg xmlns="http://www.w3.org/2000/svg" height="65vh" viewBox="0 0 586.47858 659.29778"
...
<script>
import search from './LayoutPartials/Search.vue';
...
export default {
    ...
    methods: {
        ...
        async apiGetList(showLoading = true) {

            if (showLoading) {
                this.loading = true;
            }

            let url = new URL(window.marketplace.route.list);
        ...
        async apiDetail(id) {
            let url = window.marketplace.route.detail;
            url = url.replace(':id', id);

            await fetch(url)
                .then(response => this.responseCheck(response))
                .then(data => {
                    this.$refs.pluginDetail.setProduct(data);
                });
        },
        async apiInstall(id) {
            let url = window.marketplace.route.install;
            url = url.replace(':id', id);
            ...
            await fetch(url, {
                ...
                .then(data => {
                    ...
                    this.pluginName = data.data.name;
                    this.apiGetList(false);
                });
        },
        async apiChangeStatus(name) {
            let url = window.marketplace.route.active;
        ...
        detail(product) {
            let url = window.marketplace.route.detail.replace(':id', product.id);
